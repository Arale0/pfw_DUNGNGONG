...
int	CFileVersionInfo::CompareVersion(LPCTSTR lpVersion)
{
	int iVer1[4] ={0},iVer2[4] = {0};
	iVer1[0] = (int)GetFileVersionMajor();
	iVer1[1] = (int)GetFileVersionMinor();
	iVer1[2] = (int)GetFileVersionBuild();
	iVer1[3] = (int)GetFileVersionQFE();
	swscanf_s(lpVersion,L"%d.%d.%d.%d",&iVer2[0],&iVer2[1],&iVer2[2],&iVer2[3]);
	int iRet = 0;
	for (int i = 0; i < 4; ++i)
	{
		if (iVer1[i] < iVer2[i]) iRet = -1;
		else
			if (iVer1[i] > iVer2[i]) iRet = 1;
		if (iRet != 0) break;
	}
