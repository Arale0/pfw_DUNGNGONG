...
CGNVTruyen::CGNVTruyen() :CGActivity()
{
	LOG_WRITE("NEW - CGNVTruyen()");
...
CGNVTruyen::~CGNVTruyen()
{
	LOG_WRITE("DEL - CGNVTruyen()");
}

int CGNVTruyen::Process()
{
			...
			if (pChild) {
				...
				if (czNameMainQuest.Compare(u"0") != 0) {
					...
					if (czName.Compare(u"[Truyền]") == 0) {
						...
						if (czNameAct.Find(u"Chế Linh Dược") >= 0) continue;
...
void CGNVTruyen::processActivity()
{
