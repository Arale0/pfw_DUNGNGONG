...
CGNVHuongDan::CGNVHuongDan() :CGActivity()
{
	LOG_WRITE("NEW - CGNVHuongDan()");
...
CGNVHuongDan::~CGNVHuongDan()
{
	LOG_WRITE("DEL - CGNVHuongDan()");
}

int CGNVHuongDan::Process()
{
	...
	{
		LOG_WRITE("CGNVHuongDan::Process()....");
		...
		for (i = 0; i < iCount; i++) {
			...
			if (pChild) {
				...
				if (czNameMainQuest.Compare(u"0") != 0) {
					...
					if (czName.Compare(u"[Dẫn]") == 0) {
						if (czContent.Find(u"[c][5aef8e]") >= 0) {
							if (g_pGameControl->IsButtonExitFuben()) g_pGameControl->SwitchAutoFight(TRUE);
		...
		for (int j = 0; j < iCount; j++) {
			...
			if (pChild) {
				...
				if (czNameMainQuest.Compare(u"0") != 0) {
					...
					if (czName.Compare(u"[Dẫn]") == 0) {
						if (czNameAct.Find(u"Kỹ Năng Hóa Cảnh") >= 0) continue;
						if (czNameAct.Find(u"Trật Tự Ngũ Hành") >= 0) continue;
						if (czNameAct.Find(u"Đá Quý Linh Khoáng") >= 0) continue;
...
void CGNVHuongDan::processActivity()
{
	CUE_Transform *pActivity = (CUE_Transform*)m_uActivity;
	if (!pActivity) return;
	if (m_iDelay <= 0) {
		LOG_WRITE("Click Mission...");
