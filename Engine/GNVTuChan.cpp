...
CGNVTuChan::CGNVTuChan() :CGActivity()
{
	LOG_WRITE("NEW - CGNVTuChan()");
...
CGNVTuChan::~CGNVTuChan()
{
	LOG_WRITE("DEL - CGNVTuChan()");
}

int CGNVTuChan::Process()
{
		...
		for (int j = 0; j < iCount; j++) {
			...
			if (pChild) {
				...
				if (czNameMainQuest.Compare(u"0") != 0) {
					...
					if (czName.Compare(u"[Tu]") == 0) {
						if (czNameAct.Find(u"Ma Ảo Thiên") >= 0) continue;
						if (czNameAct.Find(u"Oán Linh Môn") >= 0) continue;	
						if (czNameAct.Find(u"Hang Báu Vật") >= 0) continue;	
						if (czContent.Find(u"Vượt") >= 0) continue;	
...
void CGNVTuChan::processActivity()
{
	...
	LOG_WRITE("Clicked Misson = %d", m_iDelay);
	m_iDelay ++;
}

BOOL CGNVTuChan::isNewGuilde()
{
