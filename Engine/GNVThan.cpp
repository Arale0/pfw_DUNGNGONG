...
CGNVThan::CGNVThan() :CGActivity()
{
	LOG_WRITE("NEW - CGNVThan()");
...
CGNVThan::~CGNVThan()
{
	LOG_WRITE("DEL - CGNVThan()");
}

int CGNVThan::Process()
{
			...
			if (pChild) {
				...
				if (czNameMainQuest.Compare(u"0") != 0) {
					...
					if (czName.Compare(u"[Thần]") == 0) {
						...
						if ((czNameAct.Find(u"Thiên Mệnh Thần Dụ") >= 0) && czContent.Find(u"Chờ Ngày Mai") >= 0 ) continue;
						if (czContent.Find(u"Ngày mai") >= 0 ) continue;
